...
DELIMITER $$
USE `xmppmaster`$$
CREATE DEFINER=`root`@`localhost` FUNCTION `fs_jidusertrue`(jid char(255)) RETURNS char(255) CHARSET utf8
BEGIN
...
-- ----------------------------------------------------------------------
-- stored procedure  mmc_restart_deploy_sessionid
- -----------------------------------------------------------------------
USE `xmppmaster`;
...
DELIMITER $$
USE `xmppmaster`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `mmc_restart_deploy_sessionid`(IN IN_sessid VARCHAR(255), in force_redeploy integer, in rechedule_deploy integer)
BEGIN
-- parameters
-- IN_sessid session id deployement
-- force_redeploy = 1 do not consider the ignored status (variable: ignorestatus
-- rechedule_deploy = 1 replan the deployements between now and 1 day later.
-- The list will be updated to forbid to redeploy on those status.
...
DELIMITER ;
-- ----------------------------------------------------------------------
-- stored procedure  mmc_restart_deploy_cmdid
- -----------------------------------------------------------------------
USE `xmppmaster`;
...
DELIMITER $$
USE `xmppmaster`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `mmc_restart_deploy_cmdid`(in cmd_id integer, in force_redeploy integer, in rechedule_deploy integer)
BEGIN
set @total = 0;

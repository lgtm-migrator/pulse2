...
import pulse.utils
import pulse.package_server.imaging.api.functions
...
class ImagingMenu:
    ...
    def __init__(self, config, macaddress = None, hostname = None,uuid = None):
        ...
        if macaddress:
            assert pulse.utils.isMACAddress(macaddress)
    ...
    def _applyReplacement(self, string, condition = 'global'):
        ...
        if self.mac:
            replacements.append(
                ('##MAC##',
                 pulse.utils.reduceMACAddress(self.mac),
                 'global'))
    ...
    def write(self):
        ...
        """
        if self.mac:
            filename = os.path.join(self.config.imaging_api['base_folder'], self.config.imaging_api['bootmenus_folder'], pulse.utils.normalizeMACAddressForPXELINUX(self.mac))
        ...
        # Retreive PXE Params
        pulse.package_server.imaging.api.functions.Imaging().refreshPXEParams(_write, self)
...
class CleanMenu:
    ...
    def clear(self):
        for i in self.mac:
            if i == "": continue
            filename = os.path.join(self.config.imaging_api['base_folder'], self.config.imaging_api['bootmenus_folder'], pulse.utils.normalizeMACAddressForPXELINUX(i))
            try:
                os.unlink(filename)
            except Exception, e:
                pass
            # clear hostnamebymac
            target_file = os.path.join(self.config.imaging_api['base_folder'],self.config.imaging_api['computers_folder'], "hostnamebymac",pulse.utils.normalizeMACAddressForPXELINUX(i))
            try:
...
def changeDefaultMenuItem(macaddress, value):
    ...
    filename = os.path.join(config.imaging_api['base_folder'],
                            ...
                            pulse.utils.normalizeMACAddressForPXELINUX(macaddress))
...
class ImagingMulticastMenuBuilder:
    ...
    def __init__(self,  menu):
        ...
        #for interface in self.listnameinterface:
            #if pulse.utils.get_ip_address(interface)==self.public_ip:
    ...
    def chooseMacAddress(self):
        ...
        for k, v in self.menu['computer'].iteritems():
            if self.isValidIPv4Address(v):
                mac = pulse.utils.reduceMACAddress(k)
                filename = pulse.utils.normalizeMACAddressForPXELINUX(mac)

...
import pulse.utils
import pulse.package_server.config

# FIXME : shouldn't this cache be updated by the object itself ?

class UUIDCache(pulse.utils.Singleton):
    """
    ...
    def __init__(self):
        pulse.utils.Singleton.__init__(self)
        self.cachePath = pulse.package_server.config.P2PServerCP().imaging_api['uuid_cache_file']
        self.cacheLifetime = pulse.package_server.config.P2PServerCP().imaging_api['uuid_cache_lifetime']
    ...
    def getByMac(self, mac):
        ...
        if not pulse.utils.isMACAddress(mac):
            return False

        mac = pulse.utils.normalizeMACAddress(mac)
    ...
    def getByUUID(self, uuid):
        ...
        if not pulse.utils.isUUID(uuid):
            return False

...
import logging
import os
from pulse.xmlrpc import isTwistedEnoughForLoginPass
from pulse.utils import isUUID, subnetForIpMask

if sys.platform != "win32":
    import pwd
    import grp
    from pulse.utils import pulseConfigParser


class P2PServerCP(pulse.utils.Singleton):
    """
    Class to hold configuration directives
    """
    cacert = mmcconfdir + '/pulse/package-server/keys/cacert.pem'
    localcert = mmcconfdir + '/pulse/package-server/keys/privkey.pem'
    ...
    username = ''
    password = ''
    tmp_input_dir = '/var/lib/pulse/package-server-tmpdir'
    ...
    if sys.platform != "win32":
        ...
        umask = int('0022', 8)
        pidfile = '/var/run/pulse-package-server.pid'
    if sys.platform == "win32":

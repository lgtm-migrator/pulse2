...
# $Id$
#
# This file is part of Pulse 2, http://pulse.mandriva.org
#
...
from pulse.package_server.server import P2PSite
from pulse.package_server.description import Description
from pulse.package_server.common import Common
from pulse.package_server.mirror_api import MirrorApi
from pulse.package_server.scheduler_api import SchedulerApi
from pulse.package_server.mirror import Mirror
from pulse.package_server.package_api_get import PackageApiGet
from pulse.package_server.package_api_put import PackageApiPut
from pulse.package_server.user_package_api import UserPackageApi
from pulse.package_server.imaging.api import ImagingApi
from pulse.package_server.imaging.pxe.server import PXEProxy
...
def initialize(config):
    ...
    if len(config.mirrors) > 0:
        for mirror_params in config.mirrors:
            ...
            services.append({'type':'mirror_files', 'mp':mirror_params['mount_point']+'_files', 'server':config.public_ip, 'port':config.port, 'proto':config.proto, 'src':mirror_params['src']})
    ...
    if len(config.package_api_get) > 0:
        for mirror_params in config.package_api_get:
            ...
            services.append({'type':'package_api_get', 'mp':mirror_params['mount_point'], 'server':config.public_ip, 'port':config.port, 'proto':config.proto, 'src':mirror_params['src'], 'url':'%s://%s:%s%s'%(config.proto, config.public_ip, config.port, mirror_params['mount_point'])})
    ...
    try:
        if config.enablessl:
            pulse.xmlrpc.OpenSSLContext().setup(config.localcert, config.cacert, config.verifypeer)
            twisted.internet.reactor.listenSSL(
                port,
                P2PSite(server),
                interface = config.bind,
                contextFactory = pulse.xmlrpc.OpenSSLContext().getContext()
                )

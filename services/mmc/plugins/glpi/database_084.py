...
class Glpi084(DyngroupDatabaseHelper):
    ...
    def get_machines_list1(self, session, start, end, ctx):
        ...
        if debugfunction:
            try:
                ...
                self.logger.error(
                    "display @@@DEBUG@@@ sql literal from alchemy error : %s" % e
                )
            ...
            if displayList:
                ...
                if (
                    ...
                    join_query = join_query.outerjoin(
                        self.user, Machine.users_id == User.id
                    )
                try:
                    if regs[0]:
        ...
        if user == "root":
            ...
            for entid in entids:
                if entid.entities_id == 0 and entid.is_recursive == 1:
                    self.logger.debug(
                        "Root Entity found with recursivity = return all entities"
                    )
                    session.close()
    ...
    def getLastMachineSummaryPart(
        ...
        query = self.filterOnUUID(
            ...
            .add_column(
                self.ocslinks.c.last_ocs_update
        ...
        if count:
            ...
            for (
                ...
                last_inventory = (
                    last_ocs_update if last_ocs_update is not None else date_mod
                )
                l = [
                    ...
                    [
                        "Last Inventory Date",
                        last_inventory.strftime("%Y-%m-%d %H:%M:%S"),
                    ],
                ]
                ret.append(l)
        return ret
        ...
        if machine:
            ...
            if self.config.webservices["purge_machine"]:
                if webservice_ok:
                    ...
                    self.logger.warn(
                        "Unable to purge machine (uuid=%s) because GLPI webservice is disabled"
                        % uuid
                    )
            ...
            try:
                ...
                self.logger.warn(
                    "Unable to delete machine (uuid=%s): %s" % (uuid, str(e))
                )
                session.flush()
                session.close()
                trans.rollback()
    ...
    def get_os_for_dashboard(self, session):
        ...
        result = [
            {"os": element[2], "version": element[3], "count": 1} for element in res
        ]
    ...
    def get_computer_count_for_dashboard(self, session, count=True):
        inventory_filtered_machines = self.__filter_on(
            session.query(Machine.id).filter(
                Machine.is_deleted == 0, Machine.is_template == 0
            )
        ).all()
        ret = self.__getRestrictedComputersListQuery(None, "", session, True, False)

        inventory_filtered_machines = [
            "UUID%s" % id[0] for id in inventory_filtered_machines
        ]
        ...
        if count is True:
            ...
            return {
                "registered": inventory_filtered_machines,
                "online": registered_online_machine,
                "offline": registered_offline_machine,

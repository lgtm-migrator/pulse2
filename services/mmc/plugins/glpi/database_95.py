...
class Glpi95(DyngroupDatabaseHelper):
    ...
    def try_activation(self, config):
        ...
        try:
            ...
            logging.getLogger().error("GLPI version search 0.95 !")
            return False
    ...
    def __getRestrictedComputersListQuery(self, ctx, filt = None, session = create_session(), displayList = False, count = False):
        ...
        if filt:
            ...
            if 'uuids' in filt and isinstance(filt['uuids'], list) and len(filt['uuids']) > 0:
            ...
            if count:
                ...
                for networkport in machine.networkports:
                    ...
                    if networkport.networknames is not None:
                        ipaddresses = list({ip.name for ip in networkport.networknames.ipaddresses if ip.name != ''})
                        gateways = []
                        netmasks = []

...
def get_deploy_for_machine(
    uuidinventory, state, intervalsearch, minimum, maximum, filt
):
    """
...
def get_deploy_from_group(gid, state, intervalsearch, minimum, maximum, filt):
    ...
    return XmppMasterDatabase().get_deploy_from_group(
        gid, state, intervalsearch, minimum, maximum, filt
    )
...
def get_deploy_by_team_member(
    login, state, intervalsearch, minimum=None, maximum=None, filt=None
):
    """
    ...
    return XmppMasterDatabase().get_deploy_by_team_member(
        login, state, intervalsearch, minimum, maximum, filt
    )


def get_deploy_inprogress_by_team_member(
    login, intervalsearch, minimum=None, maximum=None, filt=None
):
    """
    ...
    return MscDatabase().get_deploy_inprogress_by_team_member(
        pulse_usersidlist, intervalsearch, minimum, maximum, filt
    )
...
def get_deploy_by_user_finished(
    ...
    if maximum == "":
        maximum = None
    return XmppMasterDatabase().get_deploy_by_user_finished(
        login, intervalsearch, minimum, maximum, filt
    )
...
def setCommand_action(target, command_id, sessionid, command_result, typemessage):
    return XmppMasterDatabase().setCommand_action(
        target, command_id, sessionid, command_result, typemessage
    )
...
def get_plugin_lists():
    ...
    result = [pluginlist, ObjectXmpp().plugindatascheduler]
    return result

...
def callXmppFunction(functionname, *args, **kwargs):
    #call fonction dans messagefilexmpp
...
#class MUCBot(slixmpp.ClientXMPP):
    #def __init__(self, conf):  # jid, password, room, nick):
        ...
        #self.xmppbrowsingpath = xmppbrowsing(
            #defaultdir=self.config.defaultdir, rootfilesystem=self.config.rootfilesystem
        #)
        ...
        #self.CYCLESCHEDULER = 4
            ...
            ## Interval between two scans for checking for new deployments (in
            ## seconds) 30 by default
            #self.schedule(
                #"deployment scan interval",
                #conf.deployment_scan_interval,
                #self.scheduledeploy,
                #repeat=True,
            #)
            ...
            #self.schedule(
                #"wol interval", 15, self.scheduledeployrecoveryjob, repeat=True
            #)
            ...
            #self.schedule(
                #"deployment end timeout",
                #conf.deployment_end_timeout,
                #self.garbagedeploy,
                #repeat=True,
            #)
        #if self.config.Boolsessionwork:
            ## Interval between two sessions checks for removing dead sessions
            ## (in seconds) 15 by default
            #self.schedule(
                #"session check",
                #conf.session_check_interval,
        ...
        #self.timecheck = 15
            ...
            #self.schedule(
                #"event leakmemory", self.timecheck, self.__leakmemory, repeat=True
            #)

        ## Interval for reloading plugins base (in seconds) 900 by default
        #self.schedule(
            #"reload plugins base",
            #conf.reload_plugins_base_interval,
            #self.loadbasepluginagent,
            #repeat=True,
        #)

        ## Interval for installing new plugins on clients (in seconds) 60 by
        ## default
        #self.schedule(
            #"remote update plugin",
            #conf.remote_update_plugin_interval,
    ...
    #def syncthingdeploy(self):
        ...
        #iddeploylist = XmppMasterDatabase().deploysyncthingxmpp()
            ...
            #for iddeploy in iddeploylist:
                ...
                #data = {"subaction": "initialisation", "iddeploy": iddeploy}
                #self.callpluginmasterfrommmc(
                    #"deploysyncthing",
                    #data,
    ...
    #def _sendwolgroup(self, listorset, nb=5000):
        ...
        #try:
            ...
            #for listsend in listforsplit:
                #self.callpluginmasterfrommmc(
                    #"wakeonlangroup", {"macadress": list(listsend)}
                #)
        #except Exception:
            #logger.error("%s" % (traceback.format_exc()))
    ...
    #def scheduledeployrecoveryjob(self):
        ...
        #wol_set = set()
        #try:
            ## machine ecart temps de deploiement terminer met status a ABORT ON
            ## TIMEOUT
            #result = XmppMasterDatabase().Timeouterrordeploy()
            #for machine in result:
                #machine_hostname = machine["jidmachine"].split("@")[0][:-4]
                #msglog.append(
                    #"<span class='log_err'>Deployment timed out on machine %s</span>"
                    #% machine_hostname
                #)
                #msglog.append(
            ...
            ## We select the machines in a deploiement stalled for more than 60
            ## seconds.
            #XmppMasterDatabase().update_status_waiting_for_deploy_on_machine_restart_or_stop()

            #msglog = []
            #machines_scheduled_deploy = XmppMasterDatabase().search_machines_from_state(
                #"DEPLOY TASK SCHEDULED"
            #)
            #for machine in machines_scheduled_deploy:
                ...
                #resultpresence = XmppMasterDatabase().getPresenceExistuuids(UUID)
                    ...
                    #msglog.append(
                        #"<span class='log_err'>Machine %s disappeared "
                        #"during deployment. GLPI ID: %s</span>"
                        #% (machine["jidmachine"], UUID)
                    #)
                    #XmppMasterDatabase().update_state_deploy(
                        #machine["id"], "ABORT MACHINE DISAPPEARED"
                    #)
                #elif resultpresence[UUID][0] == 1:
                    #XmppMasterDatabase().update_state_deploy(
                        #machine["id"], "WAITING MACHINE ONLINE"
                    #)
                #else:
            ...
            #for logmsg in msglog:
                ...
                #)
            #msglog = []
            ## We search for all the machines in the "WAITING MACHINE ONLINE"
            ## status
            #machines_waiting_online = XmppMasterDatabase().search_machines_from_state(
            ...
            #for machine in machines_waiting_online:
                #data = json.loads(machine["result"])
                    ...
                    #)
                    #XmppMasterDatabase().update_state_deploy(
                        #int(machine["id"]), "DEPLOYMENT START"
                    #)
                    ## We restart the deploiement on online machines.
                    ## We need to check if we already have a syncthing group. If
                    ## so we can decide to add it.
                    #if (
                        #"grp" in data["advanced"]
                        #and data["advanced"]["grp"] is not None
                        #and "syncthing" in data["advanced"]
                        #and data["advanced"]["syncthing"] == 1
                        #and XmppMasterDatabase().nbsyncthingdeploy(
                            #machine["group_uuid"], machine["command"]
                        #)
                        #> 2
                    #):
                        #msg = (
                            #"Starting peer deployment on machine %s"
                            #% machine["jidmachine"]
                        #)
                        #self.xmpplog(
                            #msg,
                            ...
                            #fromuser=data["login"],
                        #)
                        #XmppMasterDatabase().updatedeploytosyncthing(
                            #machine["sessionid"]
                        #)
                        ## call plugin master syncthing
                        ## initialisation deployement syncthing
                        #self.callpluginsubstitute(
                            #"deploysyncthing", data, sessionid=machine["sessionid"]
                        #)
                        #self.syncthingdeploy()
                    #else:
                        #datasession = self.session.sessiongetdata(machine["sessionid"])
                        #msglog.append(
                            #"Starting deployment on machine %s from ARS %s"
                            #% (machine["jidmachine"], machine["jid_relay"])
                        #)
    ...
    #def scheduledeploy(self):
        ...
        #for arssyncthing in list_ars_syncthing_pause:
            #datasend = {
                ...
                #"data": {"subaction": "pausefolder", "folder": arssyncthing[2]},
            #}
            #listars = arssyncthing[1].split(",")
            #for arssyncthing in listars:
                #self.send_message(
                    #mto=arssyncthing, mbody=json.dumps(datasend), mtype="chat"
                #)
        #try:
            #deploys_to_clean = XmppMasterDatabase().get_syncthing_deploy_to_clean()
            #if isinstance(deploys_to_clean, list):
                #for deploydata in deploys_to_clean:
                    #ars = XmppMasterDatabase().get_list_ars_from_cluster(
                        #deploydata["numcluster"]
                    #)
                    #datasend = {
                        #"action": "deploysyncthing",
                    ...
                    #for relay in ars:
                        #self.send_message(
                            #mto=relay["jid"], mbody=json.dumps(datasend), mtype="chat"
                        #)
                    #XmppMasterDatabase().refresh_syncthing_deploy_clean(
                        #deploydata["id"]
                    #)
        #except Exception:
            #pass
        ...
        #for deployobject in resultdeploymachine:
            ...
            #re_search = []
                ...
                #re_search = XmppMasterDatabase().getMachinedeployexistonHostname(
                    #hostname
                #)
                #if self.recover_glpi_identifier_from_name and len(re_search) == 1:
                    #update_result = XmppMasterDatabase().update_uuid_inventory(
                        #re_search[0]["id"], UUID
                    #)
                    #if update_result is not None:
                        #if update_result.rowcount > 0:
                            #logger.info(
                                #"update uuid inventory %s for machine %s"
                                #% (UUID, hostname)
                            #)
                    #resultpresence[UUID][1] = 1
                    #reloadresultpresence_uuid = (
                        #XmppMasterDatabase().getPresenceExistuuids(UUID)
                    #)
                    #resultpresence[UUID] = reloadresultpresence_uuid[UUID]
                    #self.xmpplog(
                        #"Attaching GLPI identifier [%s] in xmppmaster machine [%s]"
                        #% (UUID, hostname),
                        #type="deploy",
                ...
                #if re_search:
                    #msg.append(
                        #"<span class='log_err'>Consolidation GLPI XMPP ERROR for machine %s. "
                        #"Deployment impossible : GLPI ID is %s</span>"
                        #% (deployobject["name"], UUIDSTR)
                    #)
                    #for mach in re_search:
                        #msg.append(
                            #"<span> Action : Please check"
                            #" that mac address or serial is/are properly"
                            #" imported in GLPI: serial (%s) or macs(%s)</span>"
                            #% (mach["serial"], mach["macs"])
                        #)
                    #MSG_ERROR = "ABORT INCONSISTENT GLPI INFORMATION"
                    #sessiondeployementless = name_random(
                        #5, "glpixmppconsolidationerror"
                    #)
                #else:
                    #MSG_ERROR = "ABORT MISSING AGENT"
                    #sessiondeployementless = name_random(5, "missingagent")
                    #msg.append(
                        #"<span class='log_err'>Agent missing on machine %s. "
                        #"Deployment impossible : GLPI ID is %s</span>"
                        #% (deployobject["name"], UUIDSTR)
                    #)
                    #msg.append(
                        ...
                        #" machine %s (%s) if it is missing."
                        #% (deployobject["name"], UUIDSTR)
                    #)

                    #logging.warning(
                        #"No machine found on hostname. You must verify consolidation GLPI with xmpp"
                    #)
                    #logging.warning(
                        #"INFO\nGLPI: name %s uuid %s "
                        #% (deployobject["name"], deployobject["UUID"])
                    #)
                    #logging.warning(
                ...
                #deployobject["wol"] = 2
                    ...
                    ## If a machine is present, add deployment in deploy list to
                    ## manage.
                    #deployobject["wol"] = 0
                #else:
                    #deployobject["wol"] = 1
        ...
        #for deployuuid in self.machineDeploy:
            #try:
                ...
                #logging.debug(
                    #"send deploy on machine %s package %s"
                    #% (deployuuid, deployobject["pakkageid"])
                #)

                #self.applicationdeployjsonUuidMachineAndUuidPackage(
                    #deployuuid,
    ...
    #def start(self, event):
        ...
        #listplugins = [
            ...
            #for f in os.listdir(
                #os.path.join(
                    #os.path.dirname(os.path.realpath(__file__)), "pluginsmaster"
                #)
            #)
        ...
        #for plugin in listplugins:
            ...
            #except TypeError:
                #logging.error(
                    #"TypeError: executing plugin %s %s" % (plugin, sys.exc_info()[0])
                #)
                #logger.error("%s" % (traceback.format_exc()))
            #except Exception as e:
        ...
        #)
        #logging.debug(
            #"load plugin start. plugin for loaded plugin from pluginliststart"
        #)
        #logging.debug(
            #"================================================================"
        #)
        ...
        #for ars in XmppMasterDatabase().get_List_jid_ServerRelay_enable():
            #self.send_message(
                #mto=ars[0], mbody=json.dumps(msg_ars_init_charge), mtype="chat"
            #)
    ...
    #def changed_status(self, msg_changed_status):
        ...
        #logger.debug("%s %s" % (msg_changed_status["from"], msg_changed_status["type"]))
            ...
            #try:
                ...
                #)
                    ...
                    #for ARScluster in listrelayserver:
                        ...
                        #)
                #else:
                    #obj = XmppMasterDatabase().getcluster_resources(
                        #msg_changed_status["from"]
                    #)
                    #arscluster = []
                    ...
                    ## arscluster = list(set(arscluster))
                        ...
                        #for ars in listrelayserver:
                            #logger.debug(
                                #"removeresource on  %s for machine %s "
                                #% (ars, str(msg_changed_status["from"]))
                            #)
                            #self.send_message(
            ...
            #result = XmppMasterDatabase().initialisePresenceMachine(
                #msg_changed_status["from"], presence=1
            #)
            #if result is None or len(result) == 0:
                #return
    ...
    #def showListClient(self):
            ...
            #listrs = XmppMasterDatabase().listjidRSdeploy()
                ...
                #for i in listrs:
                    ...
                    #logger.info(
                        #"RS [%s] for deploy on %s Machine" % (i[0], len(li) - 1)
                    #)
                    #logger.debug(
                        #"{0:5}|{1:7}|{2:20}|{3:35}|{4:55}".format(
    ...
    #def applicationdeployjsonUuidMachineAndUuidPackage(
        #self,
        #uuidmachine,
        #uuidpackage,
        #idcommand,
    ...
    #):
        ...
        #name = uuidpackage
        #if name is not None:
            #return self.applicationdeployjsonuuid(
                #str(uuidmachine),
                #str(name),
                #idcommand,
                #login,
    ...
    #):
        #try:
            ...
            #)
                ...
                #)
                #logger.warn(
                    #"The following machines have the same uuid %s" % possibly_dupplicate
                #)
                #grparray = []
                #jidarray = []
                #keysyncthingarray = []
            ...
            #):
                ...
                #ARSsearch = XmppMasterDatabase().getMachinefromjid(jidrelay)
                    ...
                    #for i in range(1, len(cluster) + 1):
                        #nbars = len(cluster[i]["listarscluster"])
                                ...
                                #for logmsg in msg:
                                    ...
                                    #)
                                #logger.error(
                                    #"deploy %s error on machine %s ARS down"
                                    #% (name, uuidmachine)
                                #)
                                #return False
                            #else:
                                ...
                                #ARSsearch = XmppMasterDatabase().getMachinefromjid(
                                    #jidrelay
                                #)
                    ...
                    #found = True
                ## run deploiement
                #return self.applicationdeploymentjson(
                    #jidrelay,
                    #jidmachine,
                    #idcommand,
                    #login,
                ...
                #)
                #msg.append(
                    #"Action : The configurator must be restarted on the machine."
                #)
                #for logmsg in msg:
                    #self.xmpplog(
                        #logmsg,
    ...
    #):
        ...
        #if path is None:
            #msg.append(
                #"<span class='log_err'>Package name missing in package %s</span>"
                #% (name)
            #)
            #msg.append("Action : check the package %s" % (name))
            #XmppMasterDatabase().adddeploy(
        ...
        #descript = managepackage.loadjsonfile(os.path.join(path, "xmppdeploy.json"))
            ...
            #for logmsg in msg:
                ...
                #)
            #logger.error(
                #"deploy %s on %s  error : xmppdeploy.json missing" % (name, uuidmachine)
            #)
            #return False
    ...
    #def pluginaction(self, rep):
            ...
            #if (
                #"shell" in list(sessiondata.getdatasession().keys())
                #and sessiondata.getdatasession()["shell"]
            #):
                #self.send_message(
                    #mto=jid.JID("commandrelay@localhost"),
    ...
    #def displayData(self, data):
            ...
            #logger.info("localisationinfo : %s" % data["localisationinfo"])
                    ...
                    #logger.info(
                        #"AD found for the Machine : %s" % data["adorgbymachine"]
                    #)
                #else:
    ...
    #def loadfingerprintagentbase(self):
        ...
        #self.Update_Remote_Agentlist = Update_Remote_Agent(
            #self.config.diragentbase, self.autoupdatebool
        #)
    ...
    #def loadBasePluginList(self):
        ...
        #for element in [
            #x
            #for x in os.listdir(self.config.dirplugins)
        ...
        #]:
            ...
            ## we do not deploy a plugin with syntax error.
                ...
                #logger.error(
                    ...
                    #% os.path.join(self.config.dirplugins, element)
                #)
    ...
    #def remoteinstallPlugin(self):
        ...
        #for jidmachine in restart_machine:  # Itération pour chaque élément
            ## call one function by message to processing asynchronous tasks and
            ## can add a tempo on restart action.
    ...
    ##def register(self, iq):
        ...
        ##except IqError as e:
            ##if e.iq["error"]["code"] == "409":
                ##logger.warning("Could not register account: User already exists")
            ##else:
    ...
    #def callrestartbymaster(self, to):
        #restartmachine = {
            ...
            #"ret": 255,
        #}
        #self.send_message(mto=to, mbody=json.dumps(restartmachine), mtype="chat")
        #return True
    ...
    #def callshutdownbymaster(self, to, time=0, msg=""):
        #shutdownmachine = {
            ...
            #"ret": 0,
        #}
        #self.send_message(mto=to, mbody=json.dumps(shutdownmachine), mtype="chat")
        #return True
    ...
    #def callvncchangepermsbymaster(self, to, askpermission=1):
        ...
        #}
        #self.send_message(
            #mto=to, mbody=json.dumps(vncchangepermsonmachine), mtype="chat"
        #)
        #return True
    ...
    #def callInstallKey(self, jidAM, jidARS):
        #try:
            #body = {
                ...
                #"data": {"jidAM": jidAM},
            #}
            #self.send_message(mto=jidARS, mbody=json.dumps(body), mtype="chat")
        #except BaseException:
            #logger.error("%s" % (traceback.format_exc()))
    ...
    #def callinventory(self, to):
        #try:
            #body = {
                ...
                #"data": {"forced": "forced"},
            #}
            #self.send_message(mto=to, mbody=json.dumps(body), mtype="chat")
        #except BaseException:
            #logger.error("%s" % (traceback.format_exc()))
    ...
    #def callInstallConfGuacamole(self, torelayserver, data):
        #try:
            #body = {
                ...
                #"data": data,
            #}
            #self.send_message(mto=torelayserver, mbody=json.dumps(body), mtype="chat")
        #except BaseException:
            #logger.error("%s" % (traceback.format_exc()))
    ...
    #def sendErrorConnectionConf(self, msg):
        #reponse = {
            ...
            #"ret": 255,
        #}
        #self.send_message(mto=msg["from"], mbody=json.dumps(reponse), mtype="chat")
    ...
    #def sendrsconnectiondeploychatroom(self, to, data):
        #connection = {
            ...
            #"ret": 255,
        #}
        #self.send_message(mto=to, mbody=json.dumps(connection), mtype="chat")
    ...
    #def MessagesAgentFromChatroomConfig(self, msg):
        ...
        #try:
            ...
            ## Verify msg from chatroom master for subscription
                ...
                #data["information"] = info
                    ...
                    #data["localisationinfo"] = Localisation().geodataip(
                        #data["ippublic"]
                    #)
                #else:
        ...
        #logger.debug(
            #"Search Relay Server for "
            #"connection from user %s hostname %s localisation %s"
            #% (
        ...
        #for x in ordre:
            ## User Rule : 1
                ...
                #result1 = XmppMasterDatabase().algoruleuser(
                    #data["information"]["users"][0]
                #)
                #if len(result1) > 0:
                    ...
                    #)
                    #logger.debug(
                        #"The User Rule selects the relay server for machine %s user %s \n %s"
                        #% (
                            #data["information"]["info"]["hostname"],
                            #data["information"]["users"][0],
                            #result,
            ...
            ## Hostname Rule : 2
                ...
                #if len(result1) > 0:
                    #logger.debug(
                        #"applied rule Associate relay server based on hostname"
                    #)
                    #result = XmppMasterDatabase().IpAndPortConnectionFromServerRelay(
                        #result1[0].id
                    #)
                    #logger.debug(
                        #"The Hostname Rule selects the relay server for machine %s user %s \n %s"
                        #% (
                            #data["information"]["info"]["hostname"],
                            #data["information"]["users"][0],
                            #result,
                ...
                #try:
                    ...
                    #):
                        ...
                        #for x in result1:
                            ...
                            #if x[1] != "" and x[2] != "":
                                #distancecalculated = (
                                    #Localisation().determinationbylongitudeandip(
                        ...
                        #nbserver = len(listeserver)
                            ...
                            #logger.warn(
                                #"Geoposition Rule returned %d relay servers for machine"
                                #"%s user %s \nPossible relay servers : id list %s "
                                #% (
                                    #nbserver,
                                    #data["information"]["info"]["hostname"],
                                    #data["information"]["users"][0],
                            ...
                            #)
                            #logger.warn(
                                #"Geoposition Rule selects the relay server for machine %s user %s \n %s "
                                #% (
                                    #data["information"]["info"]["hostname"],
                                    #data["information"]["users"][0],
                                    #result,
            ...
            ## Subnet Rule : 4
                ...
                #for z in data["information"]["listipinfo"]:
                    ...
                    #)
                        ...
                        #)
                        #logger.debug(
                            #"Subnet Rule selects the relay server for machine %s user %s \n %s"
                            #% (
                                #data["information"]["info"]["hostname"],
                                #data["information"]["users"][0],
                                #result,
            ...
            #elif x[0] == 5:
                #logger.debug(
                    #"analysis the 5th rule : use default relay server %s"
            ...
            ## Load Balancer Rule : 6
                ...
                #result1 = XmppMasterDatabase().algoruleloadbalancer()
                    ...
                    #)
                    #logger.debug(
                        #"Load Balancer Rule selects the relay server for machine %s user %s \n %s"
                        #% (
                            #data["information"]["info"]["hostname"],
                            #data["information"]["users"][0],
                            #result,
            ...
            ## OU Machine Rule : 7
                ...
                #logger.debug("Analysis the 7th rule : AD organized by machines")
                    ...
                    #)
                        ...
                        #)
                        #logger.debug(
                            #"AD Rule selects the relay server for machine %s user %s \n %s"
                            #% (
                                #data["information"]["info"]["hostname"],
                                #data["information"]["users"][0],
                                #result,
            ...
            ## OU User Rule : 8
                ...
                #logger.debug("Analysis the 8th rule : AD organized by users")
                    ...
                    #)
                        ...
                        #)
                        #logger.debug(
                            #"AD organized by User rule selects relay server for machine %s user %s \n %s"
                            #% (
                                #data["information"]["info"]["hostname"],
                                #data["information"]["users"][0],
                                #result,
            ...
            ## Network Rule : 9
                ...
                #logger.debug(
                    #"Analysis rule : Associate relay server based on network address"
                #)
            ...
            ## Network Rule : 10
                ...
                ## Associates relay server based on network address
                #logger.debug(
                    #"Analysis the 10th rule : Associate relay server based on netmask adress"
                #)
        ...
        #try:
            #logger.debug(
                #" user %s and hostname %s [connection ip %s port : %s]"
                #% (
                    #data["information"]["users"][0],
                    #data["information"]["info"]["hostname"],
                    #result[0],
            ...
            #)
            #XmppMasterDatabase().log(
                #"[user %s hostanme %s] : Relay server for connection ip %s port %s"
                #% (
                    #data["information"]["users"][0],
                    #data["information"]["info"]["hostname"],
                    #result[0],
    ...
    #def unban(self, room, jid, reason=""):
        #resp = self.plugin["xep_0045"].setAffiliation(
            #room, jid, affiliation="none", reason=reason
        #)
        #return resp
    ...
    #def isInventoried(self, jid):
        ...
        #if (
            #machine["uuid_inventorymachine"] is None
            #or machine["uuid_inventorymachine"] == ""
        #):
            ...
            #uuid = ""
            #for t in results:
                #if t in self.config.blacklisted_mac_addresses:
                    #continue
                    ...
                    #computer = ComputerManager().getComputerByMac(t)
                        ...
                        #):
                            ...
                            #XmppMasterDatabase().replace_Organization_ad_id_inventory(
                                #machine["uuid_inventorymachine"], uuid
                            #)
                        #XmppMasterDatabase().updateMachineidinventory(
                            #uuid, machine["id"]
                        #)
                        #return True
                    #else:
                        #logging.getLogger().warning(
                            #"Incoherence in the address mac %s" % jid
                        #)
            #else:
                #logging.getLogger().warning(
                    #"machine [%s] not present. Cant update uuid "
                    #"in machine table. Waitting %s seconds after inject inventory"
                    #% (jid, timewaittinginventor)
                #)
                #return False
        #except Exception:
        ...
        #try:
            #data = json.loads(msg["body"])
                ...
                ## print json.dumps(data, indent=4)
                    ...
                    ## If item exists, redirects to the plugin named by the
                    ## action
                    ...
                    #data["data"]["infos"] = result
                        ...
                        #if jidmachine != "":
                            #self.send_message(
                                #mto=jidmachine, mbody=json.dumps(data), mtype="chat"
                            #)
                            ## print "send %s" % json.dumps(data)
                    #if "sendemettor" not in data["data"]:
                        #data["data"]["sendemettor"] = True
                    #if data["data"]["sendemettor"]:
                        #self.send_message(
                            #mto=msg["from"], mbody=json.dumps(data), mtype="chat"
                        #)
            ...
            ## ------------------------------------------------
                ...
                #except Exception as e:
                    #logging.error(
                        ...
                        #% (msg["from"], structinfo["action"], str(e))
                    #)
                    #logger.error("%s" % (traceback.format_exc()))
                #return True
            #return False
    ...
    #def message(self, msg):
        ...
        ## logger.debug(msg['body'])
            ...
            #return False
        #if (
            #msg["from"].bare == self.config.jidchatroommaster
            #or msg["from"].bare == self.config.confjidchatroom
        #):
            #return False
    ...
    #def callpluginmaster(self, msg):
        #try:
            #dataobj = json.loads(msg["body"])
                    ...
                    #(isinstance(dataobj["base64"], bool) and dataobj["base64"])
                    #or (
    ...
    #def createsessionfordeploydiffered(self, data):
        ...
        #self.session.createsessiondatainfo(
            #sessionid, datasession=data, timevalid=timeseconde, eventend=None
        #)
        #return sessionid
    ...
    #):
        ...
        #)
            ...
            #logging.debug("Send command and creation session")
                ...
                #self.callpluginmasterfrommmc(
                    #action, data, sessionid=command["sessionid"]
                #)
            #else:

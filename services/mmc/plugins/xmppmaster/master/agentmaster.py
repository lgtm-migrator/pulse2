...
def ObjectXmpp():
    return PluginManager().getEnabledPlugins()[
        'xmppmaster'].xmppMasterthread().xmpp
...
def calllistremotefileedit(jidmachine):
    return ObjectXmpp().iqsendpulse(
        jidmachine, {
            "action": "listremotefileedit", "data": ""}, 10)


def callremotefileeditaction(jidmachine, data, timeout=10):
    return ObjectXmpp().iqsendpulse(
        jidmachine, {
            "action": "remotefileeditaction", "data": data}, timeout)
    ...
    def __init__(self, to, action, data, timeout, precommand, postcommand):
        ...
        if self.xmpp is not None:
            ...
            self.t2 = threading.Thread(
                name=self.namethread, target=self.differed)
            self.t2.start()
        else:
...
class MUCBot(sleekxmpp.ClientXMPP):
    def __init__(self, conf):  # jid, password, room, nick):
        ...
        sleekxmpp.ClientXMPP.__init__(
            self, conf.jidagent, conf.passwordconnection)
        ...
        if self.config.Booltaskdeploy:
            ...
            self.schedule(
                'wol interval',
                15,
                self.scheduledeployrecoveryjob,
                repeat=True)
        ...
        if len(iddeploylist) != 0:
            for iddeploy in iddeploylist:
                ...
                self.callpluginmasterfrommmc(
                    "deploysyncthing", data, sessionid=name_randomplus(
                        25, pref="deploysyncthing"))
    ...
    def sendwol(self, macadress, machine_hostname=""):
        ...
        for macadressdata in listmacadress:
            if macadressdata != "":
                logging.debug(
                    "wakeonlan machine  [Machine : %s]" %
                    machine_hostname)
                self.callpluginmasterfrommmc(
                    'wakeonlan', {'macadress': macadressdata})
    ...
    def scheduledeployrecoveryjob(self):
        ...
        try:
            ...
            msglog = []
            machines_scheduled_deploy = XmppMasterDatabase(
            ).search_machines_from_state("DEPLOY TASK SCHEDULED")
            ...
            for machine in machines_wol3:
                ...
                machine_hostname = machine['jidmachine'].split('@')[0][:-4]
                msglog.append(
                    "Waiting for machine %s to be online" %
                    machine_hostname)
            for logmsg in msglog:
                self.xmpplog(logmsg,
                             type='deploy',
            ...
            for machine in machines_wol2:
                if XmppMasterDatabase().getPresenceuuid(
                        machine['inventoryuuid']):
            ...
            # status
            machines_waiting_online = XmppMasterDatabase(
            ).search_machines_from_state("WAITING MACHINE ONLINE")
            ...
            for machine in machines_waiting_online:
                ...
                if XmppMasterDatabase().getPresenceuuid(
                    ...
                    if 'grp' in data['advanced'] and data['advanced']['grp'] is not None and \
                        ...
                        datasession = self.session.sessiongetdata(
                            machine['sessionid'])
                        msglog.append(
    ...
    def scheduledeploy(self):
        ...
        try:
            # Search deploy to running
            nb_machine_select_for_deploy_cycle, resultdeploymachine = MscDatabase(
            ).deployxmpp(limitnbr=100)
        except Exception:
            logger.error("%s" % (traceback.format_exc()))
        ...
        for deployobject in resultdeploymachine:
            ...
            if resultpresence[UUID][1] == 0:
                ...
                if self.recover_glpi_identifier_from_name and len(
                        re_search) == 1:
        ...
        for deployuuid in self.machineDeploy:
            ...
            if deployobject['wol'] == 1:
                listmacadress = [x.strip()
                                 for x in deployobject['mac'].split("||")]
                for macadressdata in listmacadress:
    ...
    def start(self, event):
        ...
        listplugins = [
            re.sub(
                ...
                os.path.join(
                    os.path.dirname(
                        os.path.realpath(__file__)),
                    "pluginsmaster")) if f.startswith("plugin_auto") and f.endswith(".py")]
        logging.debug("=====================================================")
        ...
        msg_ars_init_charge = {
            'action': "cluster", 'sessionid': name_random(
                5, "clusterchargeinit"), 'data': {
                'subaction': 'startmmc'}}
                    ...
                    for t in obj['resource']:
                        if t['jidmachine'] == msg_changed_status['from']:
                            ...
                            ret = XmppMasterDatabase().updatedeploystate1(
                                t['sessionid'], "DEPLOYMENT PENDING (REBOOT/SHUTDOWN/...)")
                            if ret >= 1:
                                logger.debug(
                        ...
                        result1 = self.iqsendpulse(
                            msg_changed_status['from'], {
                                "action": "information", "data": {
                                    "listinformation": ["force_reconf"], "param": {}}}, 10)
                except Exception:
                    pass
                XmppMasterDatabase().updateMachinereconf(
                    msg_changed_status['from'])
    ...
    def applicationdeployjsonuuid(self,
        ...
        try:
            ...
            if 'error' in objmachine and objmachine['error'] == "MultipleResultsFound":
                ...
                logger.warn(
                    'getGuacamoleRelayServerMachineUuid %s' %
                    objmachine['error'])
                possibly_dupplicate = XmppMasterDatabase(
                ).get_machine_with_dupplicate_uuidinventory(uuidmachine, None)
                logger.warn(
                    "The following machines have the same uuid %s" %
                    possibly_dupplicate)
                grparray = []
                jidarray = []
                keysyncthingarray = []
            ...
            if jidmachine is not None and jidmachine != "" and jidrelay is not None and jidrelay != "":
                ...
                if ARSsearch['enabled'] == 0:
                    ...
                    msg.append(
                        ...
                        "on the machine %s to use another ARS" %
                        (name))
                    ...
                    if not found:
                        ...
                        XmppMasterDatabase().adddeploy(
                            ...
                            syncthing=0)
                        msg.append(
                            "<span class='log_err'>Alternative ARS Down</span>")
            ...
            msg.append(
                ...
                "[%s] package[%s]</span>" %
                (jidmachine, uuidmachine, name))

            if str(e) == "MultipleResultsFound":
                msg.append(
                    "<span class='log_err'>The following machines "
                    "(%s) have the same GLPI ID: %s</span>" %
                    (jidmachine, uuidmachine))
            for logmsg in msg:
                self.xmpplog(logmsg,
                             type='deploy',
                             sessionname=sessiondeployementless,
    ...
    def applicationdeploymentjson(self,
        ...
        if managepackage.getversionpackageuuid(name) is None:
            logger.error(
                "deploy %s error package name version missing" %
                (name))
            msg.append(
        ...
        if path is None:
            ...
            return False
        descript = managepackage.loadjsonfile(
            os.path.join(path, 'xmppdeploy.json'))
        ...
        if not objdeployadvanced:
            logger.error(
                "The line has_login_command for the idcommand %s is missing" %
                idcommand)
            logger.error(
                "To solve this, please remove the group, and recreate it")
        if jidmachine is not None and jidmachine != "" and jidrelay is not None and jidrelay != "":
            ...
            portpackageserver = XmppMasterDatabase(
            ).portpackageserver(userjid)[0]
        else:
            iprelay = ""
            ippackageserver = ""
            portpackageserver = ""
        ...
        data = {
            ...
            "ipmachine": XmppMasterDatabase().ipfromjid(
                jidmachine,
                None)[0],
    ...
    def loadBasePluginList(self):
        ...
        for element in [x for x in os.listdir(
                self.config.dirplugins) if x[-3:] == ".py" and x[:7] == "plugin_"]:
            ...
            if os.system("python -m py_compile \"%s\"" % element_name) == 0:
                ...
                for ligne in lignes:
                    if 'VERSION' in ligne and 'NAME' in ligne:
                        ...
                        try:
                            self.pluginagentmin[plugin['NAME']
                                                ] = plugin['VERSIONAGENT']
                        except BaseException:
    ...
    def loadPluginschedulerList(self):
        ...
        for element in os.listdir(self.config.dirschedulerplugins):
            if element.endswith('.py') and element.startswith('scheduling_'):
                f = open(
                    os.path.join(
                        self.config.dirschedulerplugins,
                        element),
                    'r')
                lignes = f.readlines()
                f.close()
        ...
        for indexplugin in range(0, len(self.file_deploy_plugin)):
            ...
            if XmppMasterDatabase().getPresencejid(plugmachine['dest']):
                if plugmachine['type'] == 'deployPlugin':
                    ...
                    self.deployPlugin(
                        plugmachine['dest'], plugmachine['plugin'])
    ...
    def register(self, iq):
        ...
        try:
            ...
            if e.iq['error']['code'] == "409":
                logger.warning(
                    "Could not register account: User already exists")
            else:
    ...
    def restartAgent(self, to):
        ...
        self.send_message(mto=to, mbody=json.dumps(
            {'action': 'restartbot', 'data': ''}), mtype='chat')

    def deployPlugin(self, jid, plugin):
        data = ''
        fichierdata = {}
        namefile = os.path.join(
            self.config.dirplugins,
            "plugin_%s.py" %
            plugin)
    ...
    def MessagesAgentFromChatroomConfig(self, msg):
        ...
        try:
            ...
            if data['action'] == 'connectionconf':
                ...
                if data['adorgbymachine'] is not None and data['adorgbymachine'] != "":
                    data['adorgbymachine'] = base64.b64decode(
                        data['adorgbymachine'])
                ...
                if data['ippublic'] is not None and data['ippublic'] != "":
                    data['localisationinfo'] = Localisation(
                    ).geodataip(data['ippublic'])
                else:
        ...
        if not (
            'information' in data and 'users' in data['information'] and len(
        ...
        for x in ordre:
            ...
            if x[0] == 1:
                ...
                if len(result1) > 0:
                    logger.debug(
                        "Applied : Associate the relay server based on user.")
            ...
            # Load Balancer Rule : 6
                ...
                if len(result1) > 0:
                    logger.debug(
                        "Applied : Rule Chooses the less requested ARS.")
                    result = XmppMasterDatabase(
                    ).IpAndPortConnectionFromServerRelay(result1[0].id)
                    logger.debug(
            ...
            # OU Machine Rule : 7
                ...
                logger.debug(
                    "Analysis the 7th rule : AD organized by machines")
            ...
            # Network Rule : 9
                ...
                networkaddress = netaddr.IPNetwork(
                    data['xmppip'] + "/" + data['xmppmask']).cidr
    ...
    def isInventoried(self, jid):
        ...
        if machine['uuid_inventorymachine'] is None or machine['uuid_inventorymachine'] == "":
            ...
            result = XmppMasterDatabase(
            ).listMacAdressforMachine(machine['id'])
        ...
        try:
            ...
            if 'action' in data and data['action'] == 'askinfo':
                ...
                if "fromplugin" in data['data']:
                    ...
                    logger.debug(
                        "Response action is calling the plugin %s" %
                        data['action'])
                else:
                    logger.warn(
                ...
                if data['data']['typeinfo'] == "info_xmppmachinebyuuid":
                    if 'host' not in data['data']:
                        logger.error(
                            "The host is missing for info_xmppmachinebyuuid")
                        return True
                    data['data']['host'] = data['data']['host'].upper()
                    data['data']['host'] = data['data']['host'].replace(
                        'UUID', '')
                    try:
        ...
        try:
            ...
            if 'type' in msgkey:
                ...
                if type == "chat" or type == "groupchat":
                    ...
                    msg.reply(
                        'Thank you, but I do not treat normal messages').send()
                    return
                elif type == 'error':
        ...
        try:
            ...
            if 'action' in dataobj and dataobj['action'] != "" and 'data' in dataobj:
                if 'base64' in dataobj and (
                    (isinstance(
                        dataobj['base64'],
                        bool) and dataobj['base64']) or (
                        isinstance(
                            dataobj['base64'],
                            str) and dataobj['base64'].lower() == 'true')):
                ...
                try:
                    logging.debug(
                        "Calling plugin %s from  %s" %
                        (dataobj['action'], msg['from']))
                    msg['body'] = dataobj
                    del dataobj['data']
